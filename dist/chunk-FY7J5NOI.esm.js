 ...
 * ====================================================
 */
import { CONFIG, isNil, isEmptyString, isNull, isBoolean, getPreciseType, isError, isServer, isPlainObject, isArray, isString, isObject, safeStableStringify, isUndefined, realValue, isNumber, hasOwnProp } from './chunk-IWMGSFDN.esm.js';
  ...
  matchesUrl(url) {
    ...
    if (matches) {
      for (const k in matches.groups) {
        matches.groups[k] = isString(matches.groups[k]) ? decodeURIComponent(matches.groups[k]) : matches.groups[k];
      }
  ...
  compile(params = {}) {
    const url = this.template.replace(
      ...
      (_match, segment, optional) => {
        ...
        return encodeURI(isString(value) ? value : "").replace(/%7C/g, "|").replace(/%25/g, "%").replace(/\$/g, "%24");
      }
    );
    let finalUrl = url;
...
\u2139\uFE0F Learn more: \`${REPO2.LINK}#absolute-url\`.`
    ...
    const _config = this.safeValidateRouterConfig(
      config || isNull(config) ? config : typeof appRoutes !== "undefined" ? appRoutes : globalThis.appRoutes
    );
...
\u2139\uFE0F More info: ${REPO2.LINK}#parameters.`
    ...
    if (isArray(params)) {
      params = params.reduce(
        (result, current, i) => segments?.[i] ? { ...result, [segments[i].name]: current } : isPlainObject(current) || isArray(current) ? { ...result, ...current } : { ...result, ...isString(current) ? { [current]: "" } : {} },
        {}
      );
  ...
  _substituteBindings(params, route2 = this._route) {
    return Object.entries(params).reduce((result, [key, value]) => {
      if (!value || typeof value !== "object" || isArray(value) || !route2?.parameterSegments.some(({ name }) => name === key)) {
...
\u2139\uFE0F Learn more: \`${REPO2.LINK}#routecurrent-optionally-accepts-parameters-as-its-second-argument-and-will-check-that-their-values-also-match-in-the-current-url\`.`
    ...
    const match = isString(current) && new RegExp(`^${name.replace(/\./g, "\\.").replace(/\*/g, ".*")}$`).test(
      current
    );
    ...
    if (Object.values(params).every((p) => !p) && !Object.values(routeParams).some((v) => !isUndefined(v))) {
      return true;
    }
...
\u2139\uFE0F More info: ${REPO2.LINK}#query-parameters.`
    ...
    return thisRoute.compile(thisParams) + stringify(
      { ...unhandled, ...thisParamsQuery },
      {
        ...
        skipNulls: true,
        encoder: (value, encoder) => isBoolean(value) ? String(+value) : encoder(value)
      }
    );
  }
  valueOf() {
...
export { RouterConfigError, route };
